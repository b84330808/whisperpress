...
| 🈶 **繁／簡體強制輸出** | 以 OpenCC 確定性轉換中文輸出（含台灣用語），不靠模型機率。 |
...
| 🪶 **自訂詞彙** | Initial prompt 提示人名、術語與輸出風格。 |
| 💻 **硬體偵測建議** | 自動偵測 CPU／記憶體／NVIDIA GPU，一鍵套用最適合的引擎與模型。 |
...
- **繁體中文輸出**：到「設定 → 轉錄 → 中文輸出字形」選「強制繁體中文」即可保證繁體（OpenCC 確定性轉換）；詞彙提示 `以下是繁體中文的逐字稿：` 則能進一步提升用詞品質。
